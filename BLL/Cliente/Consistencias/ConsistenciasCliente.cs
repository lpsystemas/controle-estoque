...
namespace BLL.Cliente.Consistencias
{
    public static class ConsistenciasCliente
    {
        ...
        {
            if (modelo.CliTipo == Convert.ToInt32(TipoClienteFornecedor.Pessoa_Fisica) && 
                !Validacao.ValidaCpf(modelo.CliCpfCnpj))
                throw new Exception("O CPF digitado é inválido. Favor digitar um numero de CPF válido!");

            if (modelo.CliTipo == Convert.ToInt32(TipoClienteFornecedor.Pessoa_Juridica) && 
                !Validacao.ValidaCnpj(modelo.CliCpfCnpj))
