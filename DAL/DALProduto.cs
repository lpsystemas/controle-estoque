  ...
  public class DALProduto : IProduto
  {
    ...
    public void Incluir(ModeloProduto modelo)
    {
      using (SqlCommand command = new SqlCommand())
      {
        ...
        command.CommandText = @"INSERT INTO PRODUTO(PRO_NOME, PRO_DESCRICAO, PRO_FOTO, PRO_VALORPAGO, PRO_VALORVENDA, 
                                                    PRO_QTDE, UMED_COD, CAT_COD, SCAT_COD) 
                                             VALUES(@NOME, @DESC, @FOTO, @VALORPAGO, @VALORVENDA, @QUANTIDADE, @UMED, 
                                                    @CATEGORIA, @SUBCAT); SELECT @@IDENTITY;";
